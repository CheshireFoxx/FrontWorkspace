...
document.querySelector("#id").onclick = function(){
    // 아이디와 일치하는 요소를 선택하는 선택자 함수
    // 1. getElememtById('아이디명')
...
document.querySelector('#tag-name').onclick = function(){
    // 태그명 기준 요소 선택
    // 1. getElementsBuName : 배열 형태의 데이터 반환
    var liArr = document.getElementsByTagName("li");

    // HTMLCollection은 생긴것은 배열이나, 배열의 유용한 메서드들은
    // 일부만 존재하는 유사배열이다.
    console.log(liArr);
...
document.querySelector("#class-name").onclick = function(){
    ...
    for (var i = 0; i < groups.length; i++) {
        var li = groups[i];
        var curent = li.style.backgroundColor;
        if(curent != 'springgreen'){
...
/*
    #userEmail을 입력하고, 확인을 눌렀을 때 사용자 입력값을 출력하세요.
    - 아이디로 접근
    - 태그명으로 접근
    - name값으로 접근
